...
int main(int argc, char **argv)
{
    ...
    /*
    if(numimg<=9)
        infile=ifstream("det/0"+ to_string(numimg+1) +".txt");
    else
        infile=ifstream("det/"+ to_string(numimg+1) +".txt");
    */
    vector<vector<cv::Rect>> obj_boxes;
    float sum=0;
    for(int j=0;j<input.size();j++)
    {
        ...
        //get img j
        frame=input[j];
        //read file j and create rect for each bound box 
        ...
        //Show true ground box
        //for(int a=0;a<realBound.size();a++)
        //    cv::rectangle(frame, realBound[a], cv::Scalar(rand()%255, rand()%255, rand()%255), 1);
